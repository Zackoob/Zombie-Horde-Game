...
func _physics_process(delta: float) -> void:
	...
	var direction : Vector3 = (player_pos + Vector3(randf(), randf(), randf()) - self.position).normalized()
	...
	var distance_player = Vector2(player_pos.x - position.x, player_pos.z - position.z)
